...
import { ToolCreateData, ToolsCreateData, ToolsUpdateData } from 'lib/types/tool'
...
export const registerTools = async (
  ...
  if(toolsIdsDeleted !== undefined) {
    await logger.apiRequest(APILogEvent.TOOL_DELETE, 'tools', toolsIdsDeleted, session)
  ...
  if(toolsCreate !== undefined) {
    ...
    if(withoutId) {
      const tools : ToolsCreateData = {
        tools: withoutId.map((toolItem) => ({
          ...
          proposedJobToTakeWithId: toolType === ToolType.TO_TAKE_WITH ? jobId : null,
        })),
      }
      if(tools.tools.length !== 0) {
        await logger.apiRequest(APILogEvent.TOOL_CREATE, 'tools', tools, session)
    ...
    if(withId) {
      const tools : ToolsUpdateData = {
        tools: withId
          .filter((toolItem) => toolItem.id && !toolsIdsDeleted?.includes(toolItem.id))
          .map((toolItem) => ({
            ...toolItem,
            proposedJobOnSiteId: toolType === ToolType.ON_SITE ? jobId : null,
            proposedJobToTakeWithId: toolType === ToolType.TO_TAKE_WITH ? jobId : null,
      ...
      if(tools.tools && tools.tools.length !== 0) {
        await logger.apiRequest(APILogEvent.TOOL_UPDATE, 'tools', tools, session)
