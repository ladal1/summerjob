...
export default function AdorationSlotsTable({ eventId }: Props) {
  const {
    data: allSlots = [],
    isLoading,
    mutate,
  ...
  const availableSlots = futureSlots.filter(
    slot => slot.workerCount < slot.capacity || slot.isUserSignedUp
  )

  // Group slots by date
  const slotsByDate = useMemo(() => {
    return availableSlots.reduce(
      (acc: Record<string, FrontendAdorationSlot[]>, slot) => {
  ...
  sortedDates.forEach(date => {
    slotsByDate[date].sort(
      (a, b) => a.localDateStart.getTime() - b.localDateStart.getTime()
    )
  })
      ...
      {isLoading ? (
        ...
        <div className="text-center py-5">
          ...
          <p className="text-muted">
            Momentálně nejsou k dispozici žádné adorační časy.
          </p>
        </div>
      ) : (
                            ...
                            <td className="text-center">
                              {slot.isUserSignedUp ? (
                                <span className="badge bg-success">
                                  Přihlášen ({slot.workerCount}/{slot.capacity})
                                </span>
                              ) : (
                                <button
                                ...
                                >
                                  {signuping === slot.id
                                    ? 'Přihlašuji...'
                                    : `Přihlásit se (${slot.workerCount}/${slot.capacity})`}
                                </button>
                              )}
                            </td>
                          </tr>
